...
| FIFO_STATUS  | 0x38 | [0]: tx_fifo_ready, [1]: rx_fifo_valid                           |
  ...
  render() {
    ...
    const currentDoc = availableDocs.find((doc) => doc.id === selectedDoc);
    ...
    return (
      <div className="doc-page">
        <div className="doc-sidebar">
          ...
          <nav className="doc-nav">
            {availableDocs.map((doc) => (
              <button
                key={doc.id}
                className={`doc-nav-item ${selectedDoc === doc.id ? "active" : ""}`}
                onClick={() => this.handleDocSelect(doc.id)}
              >
                {doc.title}
              </button>
            ))}
        ...
        <div className="doc-content">
          <h1>{currentDoc?.title || "Documentations"}</h1>
          <MarkDownEditor content={currentDoc?.content || ""} />
        </div>
      </div>
    );
  }
