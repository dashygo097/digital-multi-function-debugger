...
module uart_tx_byte_level_b115200_f100000000 (
  ...
  always @(posedge clock) begin
    ...
    if (reset) begin
      ...
      if (UART_TX_channel_ready_0 & UART_TX_channel_valid | tick) tickCounter <= 10'h0;
      else tickCounter <= tickCounter + 10'h1;
    end
    if (UART_TX_channel_ready_0) begin
...
module uart_rx_byte_level_b115200_f100000000 (
  ...
  always @(posedge clock) begin
    ...
    if (reset) begin
      ...
      if (_GEN & ~rxd) tickCounter <= 10'h1B2;
      else if (tick) tickCounter <= 10'h0;
      else tickCounter <= tickCounter + 10'h1;
    end
...
module uart_cmd_byte_level_b115200_f100000000 (
  ...
  always @(posedge clock) begin
    ...
    if (reset) begin
      ...
      if (_GEN_3) begin
        ...
        if (_GEN_5 & _GEN_4) begin
          if (_GEN_6) rx_byte_cnt <= 4'h0;
          else rx_byte_cnt <= _next_byte_cnt_T;
        end
      end
      ...
      if (_GEN_0) begin
        ...
        if (_GEN_1 & _uart_tx_UART_TX_channel_ready) begin
          if (_GEN_10) tx_byte_cnt <= 3'h0;
          else tx_byte_cnt <= tx_byte_cnt + 3'h1;
        end
      end
    ...
    if (_GEN_8) begin
    end else cmd_type_reg <= |rx_buffer_0;
    if (_GEN_9) begin
...
module axilite_master_uart_cmd_32x32_b115200_f100000000 (
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      automatic logic _GEN_8;
      automatic logic _GEN_9;
      _GEN_7 = _uart_cmd_UART_CMD_cmd_valid & ~_uart_cmd_UART_CMD_cmd_type;
      _GEN_8 = M_AXI_ARREADY & axi_arvalid;
      _GEN_9 = _GEN_0 | _GEN_1;
      if (_GEN) begin
        if (_uart_cmd_UART_CMD_cmd_valid) state <= _uart_cmd_UART_CMD_cmd_type ? 3'h4 : 3'h1;
        axi_awvalid <= _GEN_7 | axi_awvalid;
        axi_wvalid <= _GEN_7 | axi_wvalid;
        axi_arvalid <= _uart_cmd_UART_CMD_cmd_valid & _uart_cmd_UART_CMD_cmd_type | axi_arvalid;
        aw_done <= ~_GEN_7 & aw_done;
        w_done <= ~_GEN_7 & w_done;
      end else begin
        automatic logic _GEN_10 = _GEN_0 & M_AXI_AWREADY & axi_awvalid;
        automatic logic _GEN_11 = _GEN_0 & M_AXI_WREADY & axi_wvalid;
        if (_GEN_0) begin
          automatic logic _GEN_12 = aw_done & w_done;
          if (_GEN_12) state <= 3'h3;
          axi_bready <= _GEN_12 | axi_bready;
        end else begin
          if (_GEN_1) begin
            if (_GEN_2) state <= 3'h0;
          end else if (_GEN_3) begin
            if (_GEN_8) state <= 3'h5;
          end else if (_GEN_6) state <= 3'h0;
          axi_bready <= ~(_GEN_1 & _GEN_2) & axi_bready;
        end
        axi_awvalid <= ~_GEN_10 & axi_awvalid;
        axi_wvalid <= ~_GEN_11 & axi_wvalid;
        axi_arvalid <= (_GEN_9 | ~(_GEN_3 & _GEN_8)) & axi_arvalid;
        aw_done <= _GEN_10 | aw_done;
        w_done <= _GEN_11 | w_done;
      end
      if (_GEN & _GEN_7) begin
      ...
      if (~(_GEN | _GEN_9)) begin
        if (_GEN_3) axi_rready <= _GEN_8 | axi_rready;
        else axi_rready <= ~_GEN_6 & axi_rready;
      end
    end
  end  // always @(posedge)
  uart_cmd_byte_level_b115200_f100000000 uart_cmd (
      ...
      .UART_CMD_resp_valid(~(_GEN | _GEN_0) & (_GEN_1 ? _GEN_2 : ~_GEN_3 & _GEN_4 & _GEN_5)),
